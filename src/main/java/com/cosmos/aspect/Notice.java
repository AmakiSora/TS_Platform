...
public class Notice {
    ...
    @Pointcut("@annotation(com.cosmos.aspect.NoticeAOP.comment)")//评论切面
    public void comment(){}
    @AfterReturning("comment()&&args(position,text,request)")//发表评论,参数要一一对应
    public void discussNotice(String position,String text,HttpServletRequest request){
        String role = session.getAttribute("role").toString();
        if("staff".equals(role)){//用户为教师

        }else if ("student".equals(role)){//用户为学生
            String name = session.getAttribute("name").toString();
            String teaID = session.getAttribute("teacherID").toString();
            if(position.contains("_")){//有下划线为作业
                redisTemplate.opsForHash().put("notice"+teaID,"/staff/task/"+position,name+"同学发表了作业讨论");
            }else {//无则为课程
                redisTemplate.opsForHash().put("notice"+teaID,"/staff/courses/"+position,name+"同学发表了课程评价");
            }
        }else {//管理员
    ...
    public void replyNotice(String position,String text,String replier,String replierID,HttpServletRequest request){
        String name = session.getAttribute("name").toString();
        String url = request.getHeader("Referer");
        if(url.contains("courses")){//课程
            String courseID = session.getAttribute("courseID").toString();
            redisTemplate.opsForHash().put("notice"+replierID,"/notice/courses/"+courseID,name+"回复了你");
